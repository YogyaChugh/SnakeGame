...
class Snake:
    ...
    1)fetch_direction(ftemp) - Requires list of locations to choose from and returns the direction\n
    2)get_snakepos_random(map) - Requires a map object and fills the location list with a random snake position on the map\n"""
    def __init__(self,map):
        #Checks if map is an object of Map class
        if not isinstance(map,maps.Map):
            raise TypeError("Should supply a Map object :)")

        #Fetches default settings for snake and stores them
        with open(defaults_path,'r') as file:
            data = json.load(file)
        if data == {}:
            raise ValueError("Default file is empty")
        ...
        a = self.get_snakepos_random(map) #Initialize location of snake on map
        if a!=True:
            print(a)
    ...
    def get_snakepos_random(self,map):
        ...
        allowed_locations = map.get_allowed_locations()
        ...
        for i in range(self.length):
            ...
            if len(self.locations) == 0:
                ...
                print("direction here: ",temp_direction_pt)
                ...
                if (next_pos in allowed_locations) and (next_pos not in self.locations): #If the next position is even possible
                    ...
                    print("location again: ",self.locations)
                    ...
                    print("temp_direction: ",temp_direction_pt)
        return True
