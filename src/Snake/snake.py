...
class Snake:
    ...
    def __init__(self,snake_type,map_for_snake):
        ...
        if not os.path.isfile(os.path.join(paths,"assets/saved_stuff/saved_snakes.json")):
            raise ValueError("Saved snakes file either moved or renamed ! Update Code !")

        with open("saved_stuff/saved_snakes.json") as file:
            data = json.load(file)
    ...
    def draw(self,left,top,size):
        ...
        rotation = 0
        turning_dict = {((0,1),(1,0)):"/body_topright.png",
        ...
        for i in range(self.length):
            if temp == self.locations[next]:
                next+=1
                if i == 0:
                    if directions_dict[self.direction]==(0,1):
                        img_loc = "/head_down.png"
                    elif directions_dict[self.direction] == (0,-1):
                        img_loc = "/head_up.png"
                    elif directions_dict[self.direction] == (1,0):
                        img_loc = "/head_right.png"
                    else:
                        img_loc = "/head_left.png"
                elif i == self.length-1:
                    if direction==(0,1):
                        img_loc = "/tail_down.png"
                    elif direction == (0,-1):
                        img_loc = "/tail_up.png"
                    elif direction == (1,0):
                        img_loc = "/tail_right.png"
                    else:
                        img_loc = "/tail_left.png"
                else:
                    temp_direction = self.get_direction(temp,self.locations[next])
                    img_loc = turning_dict[(direction,temp_direction)]
                    direction = temp_direction
            else:
                if direction==(1,0) or direction ==(-1,0):
                    img_loc = "/body_horizontal.png"
                else:
                    img_loc = "/body_vertical.png"

            temp_image = ft.Image(self.graphics_location + img_loc,
            ...
            temp = add_tuples(temp,direction)
        return images
